...
type ActiveScreen = "leaderboard" | "homeworkUpload" | "game" | "profile";
...
export function GeeksServiceApp({
  ...
  const testRole: TestRole = "service";
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        setError(caught instanceof Error ? caught.message : "Не удалось загрузить Geeks Game");
        setState("error");
      }
    };
    void run();
...
function BottomNav({
  ...
  return (
    <nav className="bottomNav" aria-label="Geeks Game">
      <button
        type="button"
      ...
      <button
        type="button"
        className={`bottomNavButton bottomNavPrimary gameNavButton ${activeScreen === "game" ? "active" : ""}`}
        aria-label="Game"
        aria-current={activeScreen === "game" ? "page" : undefined}
        onClick={onGame}
      >
        <span className="navIcon navIconGame" aria-hidden="true">
          <svg viewBox="0 0 48 30" role="presentation">
            <path d="M13 4h22c5 0 8 3 9 8l2 10c1 4-4 7-7 4l-6-6H15l-6 6c-3 3-8 0-7-4l2-10c1-5 4-8 9-8Z" />
            <path d="M13 10v9M8.5 14.5h9M34 12h.1M39 17h.1" />
          </svg>
          <strong>Game</strong>
        </span>
      </button>
