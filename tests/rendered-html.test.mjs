...
test("ships Geeks Game page instead of the starter preview", async () => {
  ...
  assert.match(layout, /Geeks Game/);
...
test("leaderboard cards show score instead of generic TOP badges", async () => {
  ...
  assert.match(app, /GEEKS<span>GAME<\/span>/);
  assert.doesNotMatch(app, /brandRoleSwitcher/);
  ...
  assert.match(app, /activeScreen === "game"/);
  ...
  assert.match(app, /navIconGame/);
  assert.match(app, /<strong>Game<\/strong>/);
  ...
  assert.match(css, /\.navIconGame\s*{[^}]*place-items:\s*center/s);
  assert.match(css, /\.navIconGame svg\s*{[^}]*width:\s*34px/s);
  assert.match(css, /\.snakeBoard\s*{[^}]*grid-template-columns:\s*repeat\(18,\s*1fr\)/s);
